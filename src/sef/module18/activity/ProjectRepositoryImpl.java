...
public class ProjectRepositoryImpl implements ProjectRepository {
	...
	public int insertProject(Project project) throws HRSSystemException {
		...
		try {
			PreparedStatement statement = conn.prepareStatement("SELECT NEXTVAL('PROJECT_SEQ')");
			ResultSet resultSet = statement.executeQuery();
			if (resultSet.next()) {
				project.setProjectID(resultSet.getInt(1));
			}
			PreparedStatement pStmt = conn
					.prepareStatement("insert into project (ID, NAME, DESCRIPTION) VALUES (?, ?, ?)");
			...
			if (rows == 1) {
				pStmt = conn.prepareStatement("select CURRVAL('PROJECT_SEQ')");
