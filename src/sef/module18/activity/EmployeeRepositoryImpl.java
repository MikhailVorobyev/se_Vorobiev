...
public class EmployeeRepositoryImpl implements EmployeeRepository {
	...
	public Employee findEmployeeByID(int employeeID) throws HRSSystemException{
		...
		try {
			...
			throw new HRSSystemException(HRSSystemException.ERROR_FIND_EMPLOYEE_ID, e);
		}
		return result;
	}
	...
	public int insertEmployee(Employee employee) throws HRSSystemException{
		...
		try {
            PreparedStatement statement = conn.prepareStatement("SELECT NEXTVAL('EMPLOYEE_SEQ')");
			...
			PreparedStatement pStmt = conn
					.prepareStatement("insert into EMPLOYEE (ID, FIRSTNAME, LASTNAME, PROF_LEVEL) VALUES (?, ?,?,?)");
            pStmt.setInt(1, employee.getEmployeeID());
			pStmt.setString(2, employee.getFirstName());
			pStmt.setString(3, employee.getLastName());
			pStmt.setInt(4, employee.getProfLevel());

			int rows = pStmt.executeUpdate();
			conn.commit();
			if (rows == 1) {
				pStmt = conn.prepareStatement("SELECT CURRVAL('EMPLOYEE_SEQ')");
